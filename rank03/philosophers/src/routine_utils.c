...
bool	all_philosophers_full(t_setup *settings)
{
	...
	if (settings->must_eat_times == 0)
		return (false);
	safe_mutex_handle(settings->mtx_full, LOCK);
	result = (settings->fulled_phils >= settings->phils);
	safe_mutex_handle(settings->mtx_full, UNLOCK);
	return (result);
}
